...
Now `ssh bandit` connects to Level 0. For later levels you simply override the user: `ssh bandit5@bandit`. Throughout this series I refer to the host as `bndt` / `bandit` as shorthand for the full hostname — wherever you see that, substitute your own configured host.

### 3. Find the next password, then move up

On each level you will explore the home directory and the wider system to locate the next password, then SSH back in as the next user — and the next level begins.
...
5. **Work in `/tmp`.** When a level needs scratch space, create a private working directory with `mktemp -d`.
